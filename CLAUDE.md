...
**Figma 미조회 — 판단 근거.** 이 화면은 AUTH-01/02가 확립한 공유 컴포넌트(`AuthLayout` 카드 셸,
`Input`/`Button`/`FieldHint`/`PasswordChecklist`, 타이포 스케일)만으로 구성되고 텍스트 콘텐츠는
프롬프트 2~4절에 전부 명시돼 있어, 새 시각 요소(아이콘·색상·간격 토큰)를 추가로 만들 필요가 없었다 —
AUTH-02의 "확인 안 된 아이콘을 추측해서 그리지 않는다" 원칙은 검증되지 않은 새 자산을 만들 때
적용되는 것이지, 이미 검증된 공유 컴포넌트를 그대로 재사용하는 경우까지 매번 Figma를 다시 조회해야
하는 것은 아니라고 판단했다 — **다만 실제 Figma AUTH-03 프레임을 조회하지 않았으므로, 배치·문구·
상태별 시각 디테일이 디자인과 다를 수 있다는 점은 완결 필요로 남긴다.**
