...
**완결 필요(신규, 우선순위 높음) — 가입 API 연령 확인 서버 검증 (Codex P1 지적, PR 스레드 참조).** 현재
...
직접 검증하지 않았다(필드 추가 순서를 백엔드 먼저→프론트 나중으로 잡으면 이 확인 없이도 안전하다). **계약
변경 주의:** 필수 필드가 되므로 `ageConfirmed`를 보내지 않는 옛 클라이언트는 400으로 실패한다 — 배포 순서
(프론트 먼저 필드 전송, 백엔드 검증은 그 뒤) 또는 그 시점의 클라이언트 현황을 확인하라. **문서 갱신 필요:**
