...
- [ ] **`/privacy` 개정 이력·시행일 확정.** 2항이 실질적으로 바뀌었지만 v1.3 행과 시행일은 아직 없다(임의로 확정하지 않았다). 실가입을 받기 전에 시행일을 확정하고, 그 시점에 변경 고지가 필요한지 다시 확인하라 — 법적 세부는 그때 조문을 직접 확인한다.

**완결 필요(이 작업이 남긴 것):**
- **`/privacy` 개정 이력(13항)·시행일** — 2항이 실질적으로 바뀌었지만 v1.3 행과 시행일은 지성이 정한다(임의로 확정하지 않았다). 시행일이 정해지면 7일 사전 고지 예외(가입자 없음)가 여전히 유효한지부터 확인하라(위 SCR-LEGAL-01 "연령 확인 절차" 7단계 무효화 조건).
...
| **거래유형 전세/월세 구분** | UI정의서 4.4절 거래유형은 매매/전세/월세 3가지인데, search는 `dealCategory(SALE/RENT)`만 있었다 | `rentType`(JEONSE/WOLSE) 파라미터를 추가했다. **trade 값 체계(`deal_category` + `rent_type`)와 `TradeSearchRequest`의 기존 형태를 그대로 따랐다**(단일 SALE/JEONSE/WOLSE 파라미터를 새로 만들지 않음). 매매는 `dealCategory=SALE`, 전세는 `rentType=JEONSE`, 월세는 `rentType=WOLSE`로 보낸다. rentType만 보내도 전월세로 취급해 금액 범위는 `depositAmount`(보증금) 기준이다(`ComplexSearchCondition.isRent()`, TRD가 겪은 버그와 같은 규칙). 대표거래(카드의 "최근 거래가")는 거래 필터가 걸린 상관 서브쿼리로 고르므로 **선택한 유형의 최신 거래**다(IT로 확인). `dealCategory=SALE`과 `rentType`을 함께 보내면 0건이다(TRD와 같은 동작, 400 아님) | 프론트가 이미 알고 있는 trade 값 체계를 새 이름으로 감싸면 두 도메인의 파라미터가 갈라진다 | **완결 필요(아래 알려진 갭):** 카드 응답에 전세·월세 구분과 월세금액이 없다 |

**알려진 갭 — `ComplexSummaryResponse`에 `rentType`·`monthlyRentAmount`가 없다(이번에 고치지 않음).**
대표거래가 월세여도 카드에는 `representativeDealCategory=RENT`와 보증금(`representativeAmount`)만 나간다.
거래유형 필터 없이 조회하면 최신 거래가 월세인 단지가 "5,000만"처럼 보증금만 표시될 수 있다(로컬 응답
샘플에서 실제로 확인: 수원한일타운아파트). FR-3.3은 월세의 보증금과 월세를 함께 요구한다. 필드를
추가하면 `popularComplexesV2` 캐시 이름을 버전업해야 해서(위 "캐싱" 절 원칙) 이번 범위에서 뺐다. SRCH-01
카드 작업 전에 결정하라.

**성능 참고 — 지역·키워드 없이 거래유형만 거는 검색은 1.6~2.2s다(기존 문제, 이번 변경과 무관).** 대표거래
상관 서브쿼리가 단지 전체에 도는 구조 때문이다(필터 없는 조회도 같은 시간). regionCode나 keyword로
좁히면 5~300ms이고, 경기도 전체(4,781건)는 약 0.77s다. SRCH-01이 지역 없는 전국 검색을 기본으로 노출하면
NFR-1(평균 200ms)을 넘는다 — 화면 설계 때 지역 선택을 전제로 할지, 쿼리를 재설계할지 정하라.
...
반영, 형식 오류 400). `./gradlew test` 606건, `./gradlew integrationTest` 78건 전부 통과. 로컬에서 실제 코드로
...
| `popularComplexesV2::{limit}` | 인기 단지 목록 |
...
| 단지 | `/api/complexes` | `GET /search`(`regionCode`·`keyword`·`dealCategory`/`rentType`·범위 필터·`sort`·`page`/`size`), `/popular`, `/{id}`, `/map` |
...
**보존 현황(2026-09-21):** 이 화면 관련 검증 스크립트 13개(`signup-age-check`·`checkbox-focus-check`·`signup-age-server-error-check`·`signup-test`·`signup-functional`·`dup-409-check`·`server-email-error-clear-test`·`password-nickname-server-error-clear-test`·`nickname-trim-test`·`border-settle-check`·`signup-policy-newtab-check`·`privacy-age-fix-check`·`privacy-amendments-check`)가 여전히 저장소 밖 npx 캐시 경로(`%LOCALAPPDATA%\npm-cache\_npx\...`)에만 있어 **캐시 정리 시 유실될 수 있다.** 저장소 밖 로컬 경로 `C:\Users\super\homesense-e2e-scripts`에 복사해 두었다(`README.md`·`run-all.mjs`·`package.json` 포함, 저장소 미커밋 — 이 폴더도 백업은 아니고 그저 로컬 파일이다). **그중 6개(`signup-age-check`·`checkbox-focus-check`·`signup-age-server-error-check`·`signup-policy-newtab-check`·`privacy-age-fix-check`·`privacy-amendments-check`)만 실패 시 종료코드 1로 단언하고, 나머지 7개는 값을 출력만 해 종료코드 0이 곧 통과를 뜻하지 않는다** — 저장소 스위트로 옮길 때 출력 확인을 단언으로 바꿔라. `privacy-amendments-check`는 v1.2 기준(세 행을 행 단위로 검사)으로 갱신했다(2026-09-21). 이후 `privacy-withdrawal-purge-check`(방침 2·6항, 백엔드 grace-days 기본값과 대조)가 추가돼 **14개**이고 `run-all.mjs`도 갱신했다. **`signup-policy-newtab-check`가 전체 실행 중 1회 원인 미상으로 실패했다** — 스택트레이스 끝 줄만 남아 원인을 확정하지 못했고, 이후 순차 12회·병렬 18회·그 전 8회 모두 통과해 재현하지 못했다. 스크립트에서 실제 경합 하나를 찾아 고쳤다(새 탭은 `about:blank`로 시작하는데 `networkidle`만 기다리면 `/privacy` 이동 전에 `url()` 단언이 실행될 수 있어 `waitForURL(/\/privacy$/)`를 먼저 기다린다). 그것이 1회 실패의 원인이라고 확정된 것은 아니며, `run-all.mjs`는 이제 실패한 스크립트의 전체 출력을 `out/failed-*.log`에 남기므로 다시 실패하면 그 파일부터 보라.
