...
/**
 * COM-VAL-01. AUTH-02 닉네임 정책(2~12자) — UI 정의서 5.1절과 동일한 규칙을 서버 측에서도
 * 강제해 프론트엔드 검증을 우회한 요청을 막는다(NFR-9, 이중 방어선).
 *
 * <p>{@link NicknameValidator}가 null/공백을 이미 무효로 처리하므로 이 애노테이션 하나로 충분하다 —
