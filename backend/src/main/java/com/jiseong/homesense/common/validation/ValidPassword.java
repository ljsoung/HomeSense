 ...
 *
 * <p>상한(72바이트, UTF-8)은 AUTH-02 스펙 원문에는 없던 제약이다 — BCryptPasswordEncoder가 그
 * 바이트 수를 넘는 원문을 IllegalArgumentException으로 거부해(코드리뷰에서 지적됨) 그대로 두면
 * 검증은 통과하고 인코딩 단계에서 500이 새므로 {@link PasswordValidator}에 추가했다. AUTH-02
 * 스펙에도 이 상한 문구를 보강해야 한다.
 *
