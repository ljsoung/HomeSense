-- FavoriteServiceMariaDbIT 전용 최소 스키마. 테이블정의서 8장 DDL 원문 중 이 통합 테스트가 실제로
-- 건드리는 user/legal_district_code/complex/favorite_property/favorite_region 다섯 테이블만
-- 재구성한 것이다 — complex는 Complex 엔티티가 @DynamicInsert를 쓰지 않아 Hibernate가 매핑된 컬럼
-- 전부를 INSERT 문에 실으므로(ComplexRepositoryMariaDbIT의 complex-search-schema.sql과 같은 이유로)
-- 전체 컬럼을 그대로 옮겨왔다 — 권위 있는 전체 DDL은 테이블정의서 8장을 그대로 따라야 한다.
